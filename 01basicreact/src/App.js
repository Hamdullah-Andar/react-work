  ...
  return (
    <>
      ...
      <h1>Web Development/React is platform independent, we can do it in windows, linux, mac ...</h1>
        <p>We need a Editor to write our code and a browser to run our code</p>
        <p>Editor: VS Code</p>
        <p>Browser: Chrome</p>

      <h2>We need to install Node JS</h2>
        <p>As JavaScript run in a browser only, Node JS gives us the environment where we can run JavaScript</p>
        <p>as we need a compiler for every programming language, we need Node JS as a compiler/interpreter for JavaScript program</p>
        <p>React Documentation link: <a href='https://react.dev'>Rect Documentation</a></p>
        <p>nove -v will show us a our node version</p>
      
      <h2>Create github Repository</h2>
        <p>Create a github repository without readme file</p>
        <p>Run below commands in vscode</p>
          <ul>
            ...
            {/* it change the branch from master to main overriding file name */}
      ...
      <p>if we want to work on Web we will take react and react-dom</p>
      <p>if we want to work on mobile we will take react and react-native</p>
      ...
      <p>we should list the file inside the projec and make sure we have are in the folder where we have package.json file and run below command</p>
      <p>npm run start or npm start</p>
      <p>use run as there is some exception in using directly start</p>
      ...
      <p>npm run build</p>
      <p>After running build command, we will get a build folder</p>
      <p>where we get all static assets, all JavaScript files in a converted form, and this build file will be served in production</p>
      

      <h2>Clean Up Project, which have some Extra file at the begining</h2>

      <p>Ignore build file, if you have already build your project, as it is one time generated</p>

      <p>Every React project has an readme file, you can update it</p>

      <p>Most of the time we will work in src folder</p>

      <p>We will remove unnecessary file from src folder, we will remove below file</p>
      <ul>
        <li>setupTests.js</li>
      ...
      <p>Remove the comments and reportWebVitals of index.js</p>
      <p>Remove below imports</p>
      <ul>
      ...
      <p>Currently we have a function in App.js, which return an h1, and it is exported as "export default App"</p>
      <p>And it is imported in index.js and use as <App /> which in JSX</p>

      <p>let go to 01basicreact, and run it using below command</p>
      <p>npm run start</p>
